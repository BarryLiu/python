...
import requests
import re,time,random,pickle,json
...
import os
def pickle_test(cache=60):
    """ 
    保存一些代理ip
    :cache 保存的时间(s)
    """
    try:
        with open('proxys.json', encoding='utf-8') as f:
            proxy = json.load(f)
            f.close()
        if proxy is None or proxy['proxys'] is None :
            raise BaseException
        if time.time()-proxy['time'] < cache:#保存没过时间直接拿来用
            return proxy['proxys']
        # 重新查询并保存后返回 
        proxys = ['http://127.0.0.1:8888']
        with open('proxys.json', 'w', encoding='utf-8') as f:
            json.dump({'time': time.time(), 'proxys': proxys}, f)
            f.close()
        return proxys
...
if __name__ == "__main__":
    ...
    # print(pickle_test())
    b =[]
    a = [get_proxys(i) for i in range(10)]
    print(a)
    print(b)
    a = [ j for j in [get_proxys(i) for i in range(10)]]
    print(a)
    pass